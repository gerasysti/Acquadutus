object frmeMultiBairro: TfrmeMultiBairro
  Left = 0
  Top = 0
  Width = 602
  Height = 31
  TabOrder = 0
  object sbMultiBairro: TSpeedButton
    Tag = 1
    Left = 567
    Top = -1
    Width = 26
    Height = 27
    Hint = 'Seleciona o(s) Logradouro(s) ...'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      77777770F07770F0777777700077700077777777777777777777}
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    OnClick = sbMultiBairroClick
  end
  object edMultiBairro: TcxTextEdit
    Tag = 21
    Left = 0
    Top = 0
    Hint = 'Informe o(s) Bairro(s) ...'
    AutoSize = False
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    Style.StyleController = dmPrincipal.cxEditStyleNormal
    TabOrder = 0
    OnKeyDown = edMultiBairroKeyDown
    Height = 25
    Width = 567
  end
end
