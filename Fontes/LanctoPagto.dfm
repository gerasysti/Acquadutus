inherited frmLanctoPagto: TfrmLanctoPagto
  Left = 126
  Top = 50
  Caption = 'frmLanctoPagto'
  ClientHeight = 633
  ClientWidth = 1010
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  ExplicitWidth = 1016
  ExplicitHeight = 662
  PixelsPerInch = 96
  TextHeight = 13
  object gbUnidConsum: TcxGroupBox
    Left = 0
    Top = 50
    Align = alTop
    Caption = ' Unidades Consumidoras: '
    ParentFont = False
    Style.TextColor = clBlue
    Style.TextStyle = [fsBold]
    TabOrder = 0
    Height = 549
    Width = 1010
    object Label2: TLabel
      Left = 6
      Top = 524
      Width = 53
      Height = 14
      Caption = 'Endere'#231'o:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Calibri'
      Font.Style = []
      ParentFont = False
    end
    object dbg1: TcxGrid
      Tag = 2
      AlignWithMargins = True
      Left = 5
      Top = 21
      Width = 1000
      Height = 497
      Align = alTop
      PopupMenu = PopupMenu1
      TabOrder = 0
      LookAndFeel.Kind = lfFlat
      object GridDBTableView1: TcxGridDBTableView
        Navigator.Buttons.CustomButtons = <>
        Navigator.Buttons.PriorPage.Visible = True
        Navigator.Buttons.Insert.Visible = False
        Navigator.Buttons.Delete.Visible = False
        Navigator.Buttons.Edit.Visible = False
        Navigator.Buttons.Post.Visible = False
        Navigator.Buttons.Cancel.Visible = False
        Navigator.Buttons.Refresh.Visible = False
        Navigator.Buttons.SaveBookmark.Visible = False
        Navigator.Buttons.GotoBookmark.Visible = False
        Navigator.Buttons.Filter.Visible = False
        FilterBox.CustomizeDialog = False
        FilterBox.Visible = fvNever
        DataController.DataSource = ds1
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        Filtering.MRUItemsList = False
        Filtering.ColumnPopup.MaxDropDownItemCount = 50
        OptionsCustomize.ColumnFiltering = False
        OptionsCustomize.ColumnGrouping = False
        OptionsCustomize.ColumnHidingOnGrouping = False
        OptionsCustomize.ColumnMoving = False
        OptionsData.CancelOnExit = False
        OptionsData.Deleting = False
        OptionsData.DeletingConfirmation = False
        OptionsData.Editing = False
        OptionsData.Inserting = False
        OptionsSelection.CellSelect = False
        OptionsSelection.HideFocusRectOnExit = False
        OptionsSelection.HideSelection = True
        OptionsSelection.InvertSelect = False
        OptionsView.NavigatorOffset = 20
        OptionsView.NoDataToDisplayInfoText = ' '
        OptionsView.ScrollBars = ssVertical
        OptionsView.GroupByBox = False
        Styles.StyleSheet = dmPrincipal.GridTableViewStyleSheetPadrao5
        object GridDBTableView1Column5: TcxGridDBColumn
          Caption = 'Unid.Consum.'
          DataBinding.FieldName = 'R_ID_UNID_CONSUM'
          Width = 76
        end
        object cxGridDBColumn2: TcxGridDBColumn
          Tag = 1
          Caption = 'Nome / Raz'#227'o Social'
          DataBinding.FieldName = 'R_NOME_PESSOA'
          SortIndex = 0
          SortOrder = soAscending
          Width = 272
        end
        object GridDBTableView1Column4: TcxGridDBColumn
          Caption = 'Categoria'
          DataBinding.FieldName = 'R_DESCR_CATEG'
          Width = 94
        end
        object cxGridDBColumn4: TcxGridDBColumn
          Caption = 'CPF/CNPJ'
          DataBinding.FieldName = 'R_CPF_CNPJ_FTDO'
          FooterAlignmentHorz = taCenter
          Width = 114
        end
        object cxGridDBColumn1: TcxGridDBColumn
          Tag = 1
          Caption = 'Tipo Fatura'
          DataBinding.FieldName = 'R_DESCR_TIPO_FATURA'
          Width = 68
        end
        object GridDBTableView1Column6: TcxGridDBColumn
          Caption = 'Refer'#234'ncia'
          DataBinding.FieldName = 'R_REF_FTDO'
          Width = 72
        end
        object GridDBTableView1Column2: TcxGridDBColumn
          Caption = 'Vencimento'
          DataBinding.FieldName = 'R_DT_VENCIMENTO'
          HeaderAlignmentHorz = taCenter
          Width = 70
        end
        object cxGridDBColumn8: TcxGridDBColumn
          Caption = 'Valor (R$)'
          DataBinding.FieldName = 'R_VAL_FATURA'
          HeaderAlignmentHorz = taRightJustify
          Width = 74
        end
        object GridDBTableView1Column3: TcxGridDBColumn
          Caption = 'Descto (R$)'
          DataBinding.FieldName = 'R_VAL_DESCONTO'
          HeaderAlignmentHorz = taRightJustify
          Width = 68
        end
        object cxGridDBColumn5: TcxGridDBColumn
          Caption = 'Val.Pago (R$)'
          DataBinding.FieldName = 'R_VAL_PAGTO'
          HeaderAlignmentHorz = taRightJustify
          Width = 74
        end
      end
      object cxGridLevel1: TcxGridLevel
        GridView = GridDBTableView1
      end
    end
    object edEnderDescrDisitro: TcxDBTextEdit
      Left = 653
      Top = 519
      TabStop = False
      AutoSize = False
      DataBinding.DataField = 'R_ENDER_DISTRITO_DESCR'
      DataBinding.DataSource = ds1
      ParentFont = False
      Properties.CharCase = ecUpperCase
      Properties.ReadOnly = True
      Style.StyleController = dmPrincipal.cxEditStyleReadyOnly
      TabOrder = 1
      Height = 23
      Width = 160
    end
    object edEnderDescrBairro: TcxDBTextEdit
      Left = 494
      Top = 519
      TabStop = False
      AutoSize = False
      DataBinding.DataField = 'R_ENDER_BAIRRO_DESCR'
      DataBinding.DataSource = ds1
      ParentFont = False
      Properties.CharCase = ecUpperCase
      Properties.ReadOnly = True
      Style.StyleController = dmPrincipal.cxEditStyleReadyOnly
      TabOrder = 2
      Height = 23
      Width = 160
    end
    object edEnderComplemento: TcxDBTextEdit
      Tag = 2
      Left = 387
      Top = 519
      TabStop = False
      AutoSize = False
      DataBinding.DataField = 'R_ENDER_COMPLEM'
      DataBinding.DataSource = ds1
      ParentFont = False
      Properties.CharCase = ecUpperCase
      Properties.ReadOnly = True
      Style.StyleController = dmPrincipal.cxEditStyleReadyOnly
      TabOrder = 3
      Height = 23
      Width = 108
    end
    object edEnderNum: TcxDBTextEdit
      Left = 329
      Top = 519
      TabStop = False
      AutoSize = False
      DataBinding.DataField = 'R_ENDER_NUM_LETRA'
      DataBinding.DataSource = ds1
      ParentFont = False
      Properties.CharCase = ecUpperCase
      Properties.ReadOnly = True
      Style.StyleController = dmPrincipal.cxEditStyleReadyOnly
      TabOrder = 4
      Height = 23
      Width = 59
    end
    object edDescrLograd: TcxDBTextEdit
      Left = 62
      Top = 519
      TabStop = False
      AutoSize = False
      DataBinding.DataField = 'R_ENDER_LOGRAD_DESCR'
      DataBinding.DataSource = ds1
      ParentFont = False
      Properties.CharCase = ecUpperCase
      Properties.ReadOnly = True
      Style.StyleController = dmPrincipal.cxEditStyleReadyOnly
      TabOrder = 5
      Height = 23
      Width = 268
    end
    object btnDetFatAvulsa: TBitBtn
      Tag = 1
      Left = 862
      Top = 515
      Width = 145
      Height = 30
      Caption = '&Det. Fatura'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentFont = False
      TabOrder = 6
      OnClick = btnDetFatAvulsaClick
    end
  end
  object Panel: TPanel
    Left = 0
    Top = 599
    Width = 1010
    Height = 34
    Align = alBottom
    Alignment = taLeftJustify
    BevelOuter = bvNone
    Color = 14548479
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = [fsBold, fsItalic]
    ParentBackground = False
    ParentFont = False
    TabOrder = 1
    object lblTitColPesquisa: TLabel
      Left = 320
      Top = 10
      Width = 77
      Height = 13
      Alignment = taRightJustify
      AutoSize = False
      Caption = '&Pesquisar:'
      FocusControl = edPesquisa
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Calibri'
      Font.Style = []
      ParentFont = False
    end
    object edPesquisa: TcxTextEdit
      AlignWithMargins = True
      Left = 398
      Top = 6
      ParentFont = False
      Properties.CharCase = ecUpperCase
      Properties.OnChange = edPesquisaPropertiesChange
      Style.StyleController = dmPrincipal.cxEditStyleNormal
      TabOrder = 0
      Width = 195
    end
    object btnAdd: TBitBtn
      Tag = 1
      Left = 609
      Top = 3
      Width = 84
      Height = 30
      Caption = '&Novo'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentFont = False
      TabOrder = 1
      OnClick = btnAddClick
    end
    object btnEdit: TBitBtn
      Tag = 2
      Left = 692
      Top = 3
      Width = 84
      Height = 30
      Caption = '&Altera'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentFont = False
      TabOrder = 2
      OnClick = btnAddClick
    end
    object btnDel: TBitBtn
      Left = 775
      Top = 3
      Width = 84
      Height = 30
      Caption = 'E&xclui'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentFont = False
      TabOrder = 3
      OnClick = btnDelClick
    end
    object btnFechar: TBitBtn
      Left = 935
      Top = 3
      Width = 74
      Height = 30
      Caption = '&Sair'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ModalResult = 2
      ParentFont = False
      TabOrder = 4
    end
    object btnLimpar: TButton
      Tag = 3
      Left = 860
      Top = 3
      Width = 74
      Height = 30
      Caption = 'Li&mpar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 5
      OnClick = btnLimparClick
    end
  end
  object GroupBox1: TcxGroupBox
    Left = 0
    Top = 0
    Align = alTop
    ParentFont = False
    Style.Font.Charset = DEFAULT_CHARSET
    Style.Font.Color = clBlue
    Style.Font.Height = -12
    Style.Font.Name = 'Tahoma'
    Style.Font.Style = []
    Style.IsFontAssigned = True
    TabOrder = 2
    Height = 50
    Width = 1010
    object btnOk: TBitBtn
      Tag = 3
      Left = 684
      Top = 14
      Width = 56
      Height = 30
      Caption = '&OK'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 2
      OnClick = btnOkClick
    end
    object cxLabel1: TcxLabel
      Left = 775
      Top = 19
      AutoSize = False
      Caption = 'Lan'#231'tos:'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -11
      Style.Font.Name = 'Arial'
      Style.Font.Style = []
      Style.StyleController = dmPrincipal.cxLabelStyleNormal3
      Style.IsFontAssigned = True
      Properties.Alignment.Horz = taRightJustify
      Properties.Alignment.Vert = taVCenter
      Height = 18
      Width = 48
      AnchorX = 823
      AnchorY = 28
    end
    object lblQtdLancto: TcxLabel
      Left = 825
      Top = 17
      AutoSize = False
      Caption = '0'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Arial'
      Style.Font.Style = [fsBold]
      Style.StyleController = dmPrincipal.cxLabelStyleNormal3
      Style.TextColor = clBlue
      Style.IsFontAssigned = True
      Properties.Alignment.Horz = taLeftJustify
      Properties.Alignment.Vert = taVCenter
      Height = 20
      Width = 44
      AnchorY = 27
    end
    object cxLabel2: TcxLabel
      Left = 4
      Top = 18
      AutoSize = False
      Caption = 'Data do Pagto:'
      ParentFont = False
      Style.StyleController = dmPrincipal.cxLabelStyleNormal
      Properties.Alignment.Horz = taRightJustify
      Height = 17
      Width = 76
      AnchorX = 80
    end
    object edDtPagto: TcxDateEdit
      Left = 81
      Top = 17
      ParentFont = False
      Properties.InputKind = ikMask
      Style.StyleController = dmPrincipal.cxEditStyleNormal
      TabOrder = 0
      Width = 110
    end
    object cxLabel3: TcxLabel
      Left = 877
      Top = 19
      AutoSize = False
      Caption = 'Total (R$):'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -11
      Style.Font.Name = 'Arial'
      Style.Font.Style = []
      Style.StyleController = dmPrincipal.cxLabelStyleNormal3
      Style.IsFontAssigned = True
      Properties.Alignment.Horz = taRightJustify
      Properties.Alignment.Vert = taVCenter
      Height = 18
      Width = 53
      AnchorX = 930
      AnchorY = 28
    end
    object lblTotPagto: TcxLabel
      Left = 931
      Top = 17
      AutoSize = False
      Caption = '0,00'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Arial'
      Style.Font.Style = [fsBold]
      Style.StyleController = dmPrincipal.cxLabelStyleNormal3
      Style.TextColor = clBlue
      Style.IsFontAssigned = True
      Properties.Alignment.Horz = taRightJustify
      Properties.Alignment.Vert = taVCenter
      Height = 20
      Width = 72
      AnchorX = 1003
      AnchorY = 27
    end
    inline frmeLocalPagto1: TfrmeLocalPagto
      Left = 307
      Top = 16
      Width = 376
      Height = 30
      TabOrder = 1
      ExplicitLeft = 307
      ExplicitTop = 16
      ExplicitWidth = 376
      inherited sb1: TSpeedButton
        Left = 29
        ExplicitLeft = 29
      end
      inherited edId: TcxMaskEdit
        ExplicitWidth = 31
        Width = 31
      end
      inherited edDescricao: TcxTextEdit
        Left = 54
        ExplicitLeft = 54
        ExplicitWidth = 315
        Width = 315
      end
    end
    object cxLabel4: TcxLabel
      Left = 199
      Top = 19
      AutoSize = False
      Caption = 'Fonte Arrecadadora:'
      ParentFont = False
      Style.StyleController = dmPrincipal.cxLabelStyleNormal
      Properties.Alignment.Horz = taRightJustify
      Height = 17
      Width = 106
      AnchorX = 305
    end
  end
  object cds1: TClientDataSet
    Tag = 1
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'R_ID_UNID_CONSUM'
        DataType = ftInteger
      end
      item
        Name = 'R_NOME_PESSOA'
        DataType = ftString
        Size = 60
      end
      item
        Name = 'R_DESCR_TIPO_PESSOA'
        DataType = ftString
        Size = 15
      end
      item
        Name = 'R_CPF_CNPJ_FTDO'
        DataType = ftString
        Size = 18
      end
      item
        Name = 'R_DESCR_CATEG'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'R_TIPO_FATURA'
        Attributes = [faFixed]
        DataType = ftString
        Size = 1
      end
      item
        Name = 'R_DESCR_TIPO_FATURA'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'R_REFERENCIA'
        DataType = ftString
        Size = 10
      end
      item
        Name = 'R_REF_FTDO'
        DataType = ftString
        Size = 11
      end
      item
        Name = 'R_DT_PAGTO'
        DataType = ftDate
      end
      item
        Name = 'R_ID_LOCAL_PAGTO'
        DataType = ftInteger
      end
      item
        Name = 'R_DESCR_LOCAL_PAGTO'
        DataType = ftString
        Size = 40
      end
      item
        Name = 'R_DT_VENCIMENTO'
        DataType = ftDate
      end
      item
        Name = 'R_VAL_FATURA'
        DataType = ftFMTBcd
        Precision = 18
        Size = 2
      end
      item
        Name = 'R_VAL_DESCONTO'
        DataType = ftFMTBcd
        Precision = 18
        Size = 2
      end
      item
        Name = 'R_VAL_PAGTO'
        DataType = ftFMTBcd
        Precision = 18
        Size = 2
      end
      item
        Name = 'R_ENDER_LOGRAD_DESCR'
        DataType = ftString
        Size = 60
      end
      item
        Name = 'R_ENDER_NUM_LETRA'
        DataType = ftString
        Size = 7
      end
      item
        Name = 'R_ENDER_COMPLEM'
        DataType = ftString
        Size = 30
      end
      item
        Name = 'R_ENDER_BAIRRO_DESCR'
        DataType = ftString
        Size = 40
      end
      item
        Name = 'R_ENDER_DISTRITO_DESCR'
        DataType = ftString
        Size = 40
      end>
    IndexDefs = <>
    Params = <>
    ProviderName = 'prov1'
    StoreDefs = True
    AfterOpen = cds1AfterOpen
    AfterApplyUpdates = cdsEventoFixoAfterApplyUpdates
    Left = 76
    Top = 232
    object cds1R_ID_UNID_CONSUM: TIntegerField
      FieldName = 'R_ID_UNID_CONSUM'
      DisplayFormat = '0-0;0; '
    end
    object cds1R_NOME_PESSOA: TStringField
      FieldName = 'R_NOME_PESSOA'
      Size = 60
    end
    object cds1R_DESCR_TIPO_PESSOA: TStringField
      FieldName = 'R_DESCR_TIPO_PESSOA'
      Size = 15
    end
    object cds1R_CPF_CNPJ_FTDO: TStringField
      FieldName = 'R_CPF_CNPJ_FTDO'
      Size = 18
    end
    object cds1R_TIPO_FATURA: TStringField
      FieldName = 'R_TIPO_FATURA'
      FixedChar = True
      Size = 1
    end
    object cds1R_DESCR_TIPO_FATURA: TStringField
      FieldName = 'R_DESCR_TIPO_FATURA'
    end
    object cds1R_REFERENCIA: TStringField
      FieldName = 'R_REFERENCIA'
      Size = 10
    end
    object cds1R_REF_FTDO: TStringField
      FieldName = 'R_REF_FTDO'
      Size = 11
    end
    object cds1R_DT_PAGTO: TDateField
      FieldName = 'R_DT_PAGTO'
    end
    object cds1R_ID_LOCAL_PAGTO: TIntegerField
      FieldName = 'R_ID_LOCAL_PAGTO'
    end
    object cds1R_DESCR_LOCAL_PAGTO: TStringField
      FieldName = 'R_DESCR_LOCAL_PAGTO'
      Size = 40
    end
    object cds1R_DT_VENCIMENTO: TDateField
      Alignment = taCenter
      FieldName = 'R_DT_VENCIMENTO'
    end
    object cds1R_VAL_FATURA: TFMTBCDField
      FieldName = 'R_VAL_FATURA'
      currency = True
      Precision = 18
      Size = 2
    end
    object cds1R_VAL_DESCONTO: TFMTBCDField
      FieldName = 'R_VAL_DESCONTO'
      currency = True
      Precision = 18
      Size = 2
    end
    object cds1R_VAL_PAGTO: TFMTBCDField
      FieldName = 'R_VAL_PAGTO'
      currency = True
      Precision = 18
      Size = 2
    end
    object cds1R_DESCR_CATEG: TStringField
      FieldName = 'R_DESCR_CATEG'
    end
    object cds1R_ENDER_LOGRAD_DESCR: TStringField
      FieldName = 'R_ENDER_LOGRAD_DESCR'
      Size = 60
    end
    object cds1R_ENDER_NUM_LETRA: TStringField
      FieldName = 'R_ENDER_NUM_LETRA'
      Size = 7
    end
    object cds1R_ENDER_COMPLEM: TStringField
      FieldName = 'R_ENDER_COMPLEM'
    end
    object cds1R_ENDER_BAIRRO_DESCR: TStringField
      FieldName = 'R_ENDER_BAIRRO_DESCR'
      Size = 40
    end
    object cds1R_ENDER_DISTRITO_DESCR: TStringField
      FieldName = 'R_ENDER_DISTRITO_DESCR'
      Size = 40
    end
  end
  object prov1: TDataSetProvider
    DataSet = qry1
    Exported = False
    Options = [poAllowCommandText, poUseQuoteChar]
    UpdateMode = upWhereKeyOnly
    Left = 61
    Top = 208
  end
  object PopupMenu1: TPopupMenu
    Left = 216
    Top = 184
    object Novo1: TMenuItem
      Tag = 1
      Caption = '&Novo'
      OnClick = Novo1Click
    end
    object Alterar1: TMenuItem
      Tag = 2
      Caption = '&Alterar'
    end
    object Exluir1: TMenuItem
      Tag = 3
      Caption = '&Excluir'
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object Sair1: TMenuItem
      Tag = 4
      Caption = '&Sair'
    end
  end
  object qry1: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftDate
        Name = 'pDtIni'
        ParamType = ptInput
        Value = '07/01/2010'
      end
      item
        DataType = ftDate
        Name = 'pDtFim'
        ParamType = ptInput
        Value = '07/01/2010'
      end
      item
        DataType = ftString
        Name = 'pLocal'
        ParamType = ptInput
        Size = 4
        Value = ';4;'
      end>
    SQL.Strings = (
      'select'
      '  *'
      'from'
      '  sp_rel_pagto(:pDtIni,:pDtFim,:pLocal)'
      'order by'
      '  r_dt_pagto, r_id_local_pagto, r_nome_pessoa, r_referencia')
    SQLConnection = dmPrincipal.SConPrincipal
    Left = 40
    Top = 184
  end
  object ds1: TDataSource
    AutoEdit = False
    DataSet = cds1
    Left = 96
    Top = 280
  end
  object qryUsuarioPermissaoTabela: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'pIdUsuario'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'pNomeTab'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'SELECT '
      'INCLUIR,ALTERAR,EXCLUIR '
      'FROM '
      'USUARIO_PERMISSAO_TABELA'
      'WHERE'
      '(ID_USUARIO = :pIdUsuario) AND'
      '(NOME_TABELA = :pNomeTab)')
    SQLConnection = dmPrincipal.SConPrincipal
    Left = 120
    Top = 104
    object qryUsuarioPermissaoTabelaINCLUIR: TStringField
      FieldName = 'INCLUIR'
      Required = True
      FixedChar = True
      Size = 1
    end
    object qryUsuarioPermissaoTabelaALTERAR: TStringField
      FieldName = 'ALTERAR'
      Required = True
      FixedChar = True
      Size = 1
    end
    object qryUsuarioPermissaoTabelaEXCLUIR: TStringField
      FieldName = 'EXCLUIR'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object qryFatAvulsaItem: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'pIdFatura'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'select'
      '   Trim(referencia) as Ano_Mes, val_total'
      'from'
      '   fatura_avulsa_item'
      'where'
      '   (id_fatura = :pIdFatura) and'
      '   (id_item = 1)'
      'order by'
      '   referencia')
    SQLConnection = dmPrincipal.SConPrincipal
    Left = 312
    Top = 344
    object qryFatAvulsaItemANO_MES: TStringField
      FieldName = 'ANO_MES'
      Required = True
      Size = 6
    end
    object qryFatAvulsaItemVAL_TOTAL: TFMTBCDField
      FieldName = 'VAL_TOTAL'
      Precision = 18
      Size = 2
    end
  end
end
