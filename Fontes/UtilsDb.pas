...
USES SysUtils, WinTypes, WinProcs, Classes, Messages, Controls, DateUtils,
     ...
     Graphics, Db, JPEG, SqlExpr, Provider, StrUtils, Variants, cxGrid,
     cxGridDbTableView;
...
Procedure PesquisaIncremental(sTexto: String; oDataSource: TDataSource; sCampoPesq: string);
...
//Function NavegaClient(wKey: Word; cds1: TClientDataSet): Word;
...
Function DtEmissaoMesAnterior(iBairro: Integer; sAnoMes: String): String;
Function CriaCampoTab(sTab,sCampo,sTipo: string): Boolean;
Function CPF_Existe(sCPF_CNPJ,sMsg: string): Boolean;
...
Begin
   ...
   Try
      try
         oQry1.Open;
         if not oQry1.FieldByName('ULT_ID').IsNull then
...
Begin
   ...
   oQry1.SQLConnection := dmPrincipal.SConPrincipal;
   oQry1.SQL.Clear;
   sSQL_Text := 'SELECT '+UpperCase(sCampoRet)+' FROM '+UpperCase(sTabela)+
                          ' WHERE '+UpperCase(sCampoPesq)+' = :Param1';
   oQry1.SQL.Add(sSQL_Text);
   oQry1.Params[0].Value := sTextPesq;
   Try
     oQry1.Open;
     Try
         Result := oQry1.Fields[0].AsString;
     except
         Result := '';
     End;
     If Result = '' Then
...
Begin
   Result := True;
   //oTab.DisableControls;
   //bmRegAtual := oTab.GetBookmark;
   If oTab.State in [dsInsert,dsEdit] Then
      oTab.Post;
   try
      oTab.ApplyUpdates(0);
   except
      Mensagem('Não foi possível Gravar essa Transação no Banco de Dados !',
   ...
   Begin
      oTab.Close;
      oTab.Open;
      oTab.GotoBookmark(bmRegAtual);
   End;
   oTab.EnableControls;
   oTab.FreeBookmark(bmRegAtual);}
End;
...
Function DtEmissaoMesAnterior(iBairro: Integer; sAnoMes: String): String;
Var
   sAnoMesAnterior: String[6];
   sEmissaoMesAnter: String[10];
   dEmissaoMesAnter: TDate;
   oQry1: TSQLQuery;
Begin
   Result := '';
   sAnoMesAnterior := DecMesAno(sAnoMes,'2');
   oQry1 := TSQLQuery.Create(Nil);
   oQry1.SQLConnection := dmPrincipal.SConPrincipal;
   oQry1.SQL.Add('SELECT Dt_Emissao FROM Calc_fatura_mensal_resumo');
   oQry1.SQL.Add('WHERE (Ano_Mes = '+QuotedStr(sAnoMesAnterior)+') AND (id_Bairro = '+IntToStr(iBairro)+')');
   Try
      oQry1.Open;
      if oQry1.Fields[0].IsNull then
      Begin
         sEmissaoMesAnter := '01/'+MesStr(sAnoMes)+'/'+AnoStr(sAnoMes);
         dEmissaoMesAnter := StrToDate(sEmissaoMesAnter);
      End Else
      Begin
         dEmissaoMesAnter := IncDay(oQry1.Fields[0].Value,1);
      End;
      Result := DateToStr(dEmissaoMesAnter);
   Finally
      oQry1.Close;
      FreeAndNil(oQry1);
   End;
...
begin
  ...
  Result := False;
  sNomeRazao := Pesquisa('PESSOA','CPF_CNPJ',sCPF_CNPJ,'NOME_RAZAO_SOCIAL','');
  if sNomeRazao <> '' then
  begin
     if sMsg = 'S' then
        Mensagem('CPF / CNPJ já existe ...'+#13+
           sNomeRazao,'Aviso !!!',MB_OK+MB_ICONEXCLAMATION);
     Result := True;
  end;
