...
Uses
 ...
 Forms, Dialogs, stdCtrls, DBClient, cxGridTableView,
 Graphics, Db, SqlExpr, JPEG, Mask, DbCtrls, ExtCtrls, ShellApi,
 cxDropDownEdit, cxGrid;
...
Function DescrMesAno(sAnoMes: String): String;
...
Function GetUltimaVersao(sExe: string): string;
...
Procedure PreecheComboUF(oCombo: TcxComboBox);
Procedure FindItemCombo(oCombo: TcxComboBox; sTexto: string);

implementation

uses Variants, StrUtils, udmPrincipal, VarGlobais, UtilsDb, Windows;
...
Begin
 If sSituac = '0' Then
   Result := 'AGAURDANDO LIGA«√O'
 else If sSituac = '1' Then
   Result := 'ATIVA'
 Else If sSituac = '2' Then
   Result := 'CORTADA'
 Else If sSituac = '3' Then
   Result := 'DESATIVADA'
 Else
   Result := 'ISENTA';
End;
...
begin
  ...
  Result := PreencheZeros(IntToStr(iQtdAnos),2)+PreencheZeros(IntToStr(iQtdMeses),2);
...
Function GetPathRel: WideString;
Var
  sPahtAplicacao: WideString;
  tfArq: TextFile;
  sLinha: WideString;
  sPathRel: WideString;
begin
  sPahtAplicacao := ExtractFilePath(ParamStr(0));
  If FileExists(sPahtAplicacao+'\ACQUADUTUS.INI') Then
  Begin
     AssignFile(tfArq,sPahtAplicacao+'\ACQUADUTUS.INI');
     Reset(tfArq);
     while not eof(tfArq) do
     begin
       Readln(tfArq,sLinha);
       if UpperCase(sLinha) = '[REL]' then
       begin
          Readln(tfArq,sLinha);
          break;
       end;
...
begin
  ...
  Begin
     ...
     begin
       ...
       if UpperCase(sLinReg) = '[ULT_ANO_MES]' then
       begin
          Readln(tfArq,sLinReg);
          break;
       end;
...
Begin
   if iTam = 5 then
   begin
      sCodigo := PreencheZeros(sCodigo,5);
      Try
        iTotNum  := (StrToInt(sCodigo[5]) * 2) +
                    (StrToInt(sCodigo[4]) * 3) +
                    (StrToInt(sCodigo[3]) * 4) +
                    (StrToInt(sCodigo[2]) * 5) +
                    (StrToInt(sCodigo[1]) * 6);
        Result  := IntToStr(iTotNum Mod 10);
      Except
        Result   := '0';
      End;
   ...
   begin
      sCodigo := PreencheZeros(sCodigo,7);
      Try
        iTotNum  := (StrToInt(sCodigo[7]) * 3) +
                    (StrToInt(sCodigo[6]) * 4) +
                    (StrToInt(sCodigo[5]) * 5) +
                    (StrToInt(sCodigo[4]) * 6) +
                    (StrToInt(sCodigo[3]) * 7) +
                    (StrToInt(sCodigo[2]) * 8) +
                    (StrToInt(sCodigo[1]) * 9);
        Result  := IntToStr(iTotNum Mod 10);
      Except
        Result   := '0';
      End;
...
Begin
   lDois   := True;
   iTotal  := 0;
   wLenText:= Length(sText);
   For wConta := wLenText DownTo 1 Do
   Begin
       If lDois Then
          sProduto := IntToStr((StrToInt(Copy(sText,wConta,1))*2))
       Else
          sProduto := IntToStr((StrToInt(Copy(sText,wConta,1))*1));
       For wConta2 := 1 To Length(sProduto) Do
          iTotal:= iTotal + StrToInt(Copy(sProduto,wConta2,1));
       lDois := Not lDois;
   End;
   wResto := iTotal Mod 10;
   If wResto = 0 Then
      Result := '0'
   Else
      Result := IntToStr(10-wResto);
End;
...
begin
  ...
  sl1 := TStringList.Create;
  sl1.LoadFromFile(sPathExe+'\ACQUADUTUS.INI');
  Result := '';
...
begin
  ...
  for iConta := 0 to 27 do
    oCombo.Properties.items.add(glb_aUF[iConta]);
  oCombo.ItemIndex := 0;
...
Var
  iConta: Integer;
begin
  for iConta := 0 to oCombo.Properties.Items.Count - 1 do
    if oCombo.properties.items[iConta] = sTexto then
       oCombo.ItemIndex := iConta;
end;
