inherited frmDetalhaFatura: TfrmDetalhaFatura
  Left = 182
  Top = 148
  Caption = 'DETALHAMENTO DA FATURA ...'
  ClientHeight = 561
  ClientWidth = 786
  OnClose = FormClose
  OnShow = FormShow
  ExplicitWidth = 792
  ExplicitHeight = 590
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 527
    Width = 786
    Height = 34
    Align = alBottom
    Alignment = taLeftJustify
    BevelOuter = bvNone
    Color = 12886916
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMaroon
    Font.Height = -16
    Font.Name = 'Calibri'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
    object BitBtn5: TBitBtn
      Tag = 2
      Left = 692
      Top = 2
      Width = 93
      Height = 30
      Caption = '&Sair'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentFont = False
      TabOrder = 0
      OnClick = BitBtn5Click
    end
    object cxLabel10: TcxLabel
      Left = 4
      Top = 5
      AutoSize = False
      Caption = 'D'#233'bitos (R$):'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWhite
      Style.Font.Height = -12
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.TextColor = clRed
      Style.IsFontAssigned = True
      Properties.Alignment.Horz = taRightJustify
      Properties.Alignment.Vert = taVCenter
      Height = 20
      Width = 87
      AnchorX = 91
      AnchorY = 15
    end
    object cxLabel11: TcxLabel
      Left = 221
      Top = 6
      AutoSize = False
      Caption = 'Cr'#233'ditos (R$):'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWhite
      Style.Font.Height = -12
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.TextColor = clBlue
      Style.IsFontAssigned = True
      Properties.Alignment.Horz = taRightJustify
      Properties.Alignment.Vert = taVCenter
      Height = 19
      Width = 93
      AnchorX = 314
      AnchorY = 16
    end
    object edValTotal: TcxCurrencyEdit
      Left = 508
      Top = 3
      TabStop = False
      AutoSize = False
      ParentFont = False
      Properties.Alignment.Horz = taRightJustify
      Properties.DecimalPlaces = 2
      Properties.DisplayFormat = ',0.00'
      Properties.EditFormat = ',0.00'
      Properties.ReadOnly = True
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clMaroon
      Style.Font.Height = -15
      Style.Font.Name = 'Calibri'
      Style.Font.Style = [fsBold]
      Style.TextColor = clRed
      Style.IsFontAssigned = True
      TabOrder = 3
      Height = 27
      Width = 99
    end
    object cxLabel1: TcxLabel
      Left = 427
      Top = 6
      AutoSize = False
      Caption = 'Total (R$):'
      ParentFont = False
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clWhite
      Style.Font.Height = -12
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = [fsBold]
      Style.TextColor = clRed
      Style.IsFontAssigned = True
      Properties.Alignment.Horz = taRightJustify
      Properties.Alignment.Vert = taVCenter
      Height = 19
      Width = 79
      AnchorX = 506
      AnchorY = 16
    end
    object edCreditos: TcxCurrencyEdit
      Left = 316
      Top = 3
      TabStop = False
      AutoSize = False
      ParentFont = False
      Properties.Alignment.Horz = taRightJustify
      Properties.DecimalPlaces = 2
      Properties.DisplayFormat = ',0.00'
      Properties.EditFormat = ',0.00'
      Properties.ReadOnly = True
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clMaroon
      Style.Font.Height = -15
      Style.Font.Name = 'Calibri'
      Style.Font.Style = [fsBold]
      Style.TextColor = clBlue
      Style.IsFontAssigned = True
      TabOrder = 5
      Height = 27
      Width = 99
    end
    object edDebitos: TcxCurrencyEdit
      Left = 92
      Top = 3
      TabStop = False
      AutoSize = False
      ParentFont = False
      Properties.Alignment.Horz = taRightJustify
      Properties.DecimalPlaces = 2
      Properties.DisplayFormat = ',0.00'
      Properties.EditFormat = ',0.00'
      Properties.ReadOnly = True
      Style.Font.Charset = DEFAULT_CHARSET
      Style.Font.Color = clMaroon
      Style.Font.Height = -15
      Style.Font.Name = 'Calibri'
      Style.Font.Style = [fsBold]
      Style.TextColor = clRed
      Style.IsFontAssigned = True
      TabOrder = 6
      Height = 27
      Width = 99
    end
  end
  object GridTable: TcxGrid
    Left = 0
    Top = 0
    Width = 786
    Height = 528
    Margins.Left = 1
    Margins.Top = 1
    Margins.Right = 1
    Margins.Bottom = 1
    Align = alTop
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Calibri'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    LookAndFeel.Kind = lfFlat
    object GridTableDBTableView1: TcxGridDBTableView
      Navigator.Buttons.CustomButtons = <>
      Navigator.Buttons.PriorPage.Visible = True
      Navigator.Buttons.Insert.Visible = False
      Navigator.Buttons.Delete.Visible = False
      Navigator.Buttons.Edit.Visible = False
      Navigator.Buttons.Post.Visible = False
      Navigator.Buttons.Cancel.Visible = False
      Navigator.Buttons.Refresh.Visible = False
      Navigator.Buttons.SaveBookmark.Visible = False
      Navigator.Buttons.GotoBookmark.Visible = False
      Navigator.Buttons.Filter.Visible = False
      FilterBox.CustomizeDialog = False
      FilterBox.Visible = fvNever
      DataController.DataSource = dsDetFatura
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      Filtering.MRUItemsList = False
      Filtering.ColumnPopup.MaxDropDownItemCount = 50
      OptionsCustomize.ColumnFiltering = False
      OptionsCustomize.ColumnGrouping = False
      OptionsCustomize.ColumnHidingOnGrouping = False
      OptionsCustomize.ColumnMoving = False
      OptionsData.CancelOnExit = False
      OptionsData.Deleting = False
      OptionsData.DeletingConfirmation = False
      OptionsData.Editing = False
      OptionsData.Inserting = False
      OptionsSelection.CellSelect = False
      OptionsSelection.HideFocusRectOnExit = False
      OptionsSelection.InvertSelect = False
      OptionsSelection.UnselectFocusedRecordOnExit = False
      OptionsView.NavigatorOffset = 20
      OptionsView.NoDataToDisplayInfoText = ' '
      OptionsView.ScrollBars = ssVertical
      OptionsView.ExpandButtonsForEmptyDetails = False
      OptionsView.GridLineColor = clBtnFace
      OptionsView.GroupByBox = False
      Styles.StyleSheet = dmPrincipal.GridTableViewStyleSheetPadrao2
      object GridTableDBTableView1Column7: TcxGridDBColumn
        Caption = #205'tem'
        DataBinding.FieldName = 'R_ID'
        HeaderAlignmentHorz = taRightJustify
        Width = 35
      end
      object GridTableDBTableView1Column1: TcxGridDBColumn
        Caption = 'Descri'#231#227'o'
        DataBinding.FieldName = 'R_DESCRICAO'
        Width = 500
      end
      object GridTableDBTableView1Column8: TcxGridDBColumn
        Caption = 'Refer'#234'ncia'
        DataBinding.FieldName = 'R_REFERENCIA'
        Width = 114
      end
      object GridTableDBTableView1Column2: TcxGridDBColumn
        Caption = 'Valor (R$)'
        DataBinding.FieldName = 'R_VALOR'
        HeaderAlignmentHorz = taRightJustify
        Width = 91
      end
      object GridTableDBTableView1Column3: TcxGridDBColumn
        Caption = 'Tipo'
        DataBinding.FieldName = 'R_TIPO'
        Width = 31
      end
    end
    object GridTableLevel1: TcxGridLevel
      GridView = GridTableDBTableView1
    end
  end
  object qryDetFatura: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'pUnidConsum'
        ParamType = ptInput
        Value = '26'
      end
      item
        DataType = ftString
        Name = 'pAnoMes'
        ParamType = ptInput
      end>
    SQL.Strings = (
      
        'select * from sp_detalha_fatura(:pUnidConsum,:pAnoMes) order by ' +
        'r_id')
    SQLConnection = dmPrincipal.SConPrincipal
    Left = 432
    Top = 216
    object qryDetFaturaR_ID: TIntegerField
      FieldName = 'R_ID'
    end
    object qryDetFaturaR_REFERENCIA: TStringField
      FieldName = 'R_REFERENCIA'
      Size = 30
    end
    object qryDetFaturaR_DESCRICAO: TStringField
      FieldName = 'R_DESCRICAO'
      Size = 140
    end
    object qryDetFaturaR_VALOR: TFMTBCDField
      FieldName = 'R_VALOR'
      Precision = 18
      Size = 2
    end
    object qryDetFaturaR_TIPO: TStringField
      FieldName = 'R_TIPO'
      FixedChar = True
      Size = 1
    end
  end
  object provDetFatura: TDataSetProvider
    DataSet = qryDetFatura
    Left = 448
    Top = 240
  end
  object cdsDetFatura: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'provDetFatura'
    Left = 464
    Top = 280
    object cdsDetFaturaR_ID: TIntegerField
      FieldName = 'R_ID'
    end
    object cdsDetFaturaR_REFERENCIA: TStringField
      FieldName = 'R_REFERENCIA'
      Size = 30
    end
    object cdsDetFaturaR_DESCRICAO: TStringField
      FieldName = 'R_DESCRICAO'
      Size = 140
    end
    object cdsDetFaturaR_VALOR: TFMTBCDField
      FieldName = 'R_VALOR'
      currency = True
      Precision = 18
      Size = 2
    end
    object cdsDetFaturaR_TIPO: TStringField
      Alignment = taCenter
      FieldName = 'R_TIPO'
      FixedChar = True
      Size = 1
    end
  end
  object dsDetFatura: TDataSource
    AutoEdit = False
    DataSet = cdsDetFatura
    Left = 480
    Top = 320
  end
end
