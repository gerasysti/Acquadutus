object frmeMultiLogradouro: TfrmeMultiLogradouro
  Left = 0
  Top = 0
  Width = 602
  Height = 28
  TabOrder = 0
  object sbMultiLograd: TSpeedButton
    Tag = 1
    Left = 566
    Top = -1
    Width = 26
    Height = 28
    Hint = 'Seleciona o(s) Logradouro(s) ...'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      77777770F07770F0777777700077700077777777777777777777}
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    OnClick = sbMultiLogradClick
  end
  object edMultiLograd: TcxTextEdit
    Tag = 21
    Left = 0
    Top = 1
    Hint = 'Informe o(s) Logradouro(s) ...'
    AutoSize = False
    ParentFont = False
    ParentShowHint = False
    Properties.MaxLength = 1024
    ShowHint = True
    Style.StyleController = dmPrincipal.cxEditStyleNormal
    TabOrder = 0
    OnKeyDown = edMultiLogradKeyDown
    OnKeyPress = edMultiLogradKeyPress
    Height = 25
    Width = 566
  end
end
